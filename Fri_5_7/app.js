(function(){
  ...
  function App(){
    ...
    function resolveQuestionBankUrl() {
      const pageUrl = new URL(window.location.href);
      const isHtml = /\.html?$/i.test(pageUrl.pathname);
      const baseFolderUrl = (() => {
        if (pageUrl.pathname.endsWith("/")) return pageUrl;
        if (isHtml) return new URL(".", pageUrl);
        return new URL(pageUrl.href + "/");
      })();
      return new URL("question_bank.json", baseFolderUrl).toString();
    }

    // Fetch question bank on mount
    useEffect(() => {
      const url = resolveQuestionBankUrl();
      setLoadedUrl(url);
        ...
        .catch(err => {
          ...
          const extra =
            window.location.protocol === "file:"
              ? "\n\nTip: if you opened this page as a file (file://), browsers block fetch(). Run a local web server instead (e.g. VS Code Live Server) and open the http:// URL."
              : "";
          alert(
            "Could not load question_bank.json or one of its sets.\n\nTried:\n" +
              url +
              extra
          );
        });
